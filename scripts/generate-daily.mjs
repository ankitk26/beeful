...
const allPangrams = JSON.parse(fs.readFileSync('scripts/pangrams.json', 'utf8'));
...
for (let i = 0; i < 365; i++) {
    ...
    while (attempt < 500) {
        ...
        const seed = dateStr.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) + attempt * 17;
...
console.log('Successfully generated daily puzzles and hashed valid words. Each game has ≥30 words and ≥2 pangrams.');
